...
- **Always build the release after bug fixes and major milestones** — the user tests every change via the live app, so each fix/feature must produce a fresh binary:
  1. `npm run build` (frontend → `dist/`)
  2. `source ~/.cargo/env && cargo build --release` (from `src-tauri/`)
  3. The user then tests through their shell launcher (`~/.local/share/applications/rhystic-tracker.desktop` → `launch.sh` → `src-tauri/target/release/rhystic-tracker`).
...
- Suggested flow:
  ...
  3. Push to `origin master` — note the local branch is `master` but the remote default is `main`, so push explicitly: `git push origin HEAD:main`.
  4. Build the release (see build step above).
- Never commit:
