...
import { GeneralTab, AppearanceTab, ConnectionTab, StorageTab, AboutTab, MTG_COLORS, BG_WINDOWS, SettingsTab } from './settings';
        ...
        <div className="max-w-4xl w-full space-y-6">
          ...
          {(!isSearching ? activeTab === 'appearance' : (matchThemes || matchGlassOpacity || matchManaPipStyle || matchCollectionSort || matchCompactMode || matchDeckFlair || matchBackground)) && (
            <AppearanceTab
              palette={palette}
              isSearching={isSearching}
              matchThemes={matchThemes}
              matchGlassOpacity={matchGlassOpacity}
              matchManaPipStyle={matchManaPipStyle}
              matchCollectionSort={matchCollectionSort}
              matchCompactMode={matchCompactMode}
              matchDeckFlair={matchDeckFlair}
              matchBackground={matchBackground}
              activeThemeId={activeThemeId}
              setActiveThemeId={setActiveThemeId}
              manaThemeOptions={manaThemeOptions}
              glassOpacity={glassOpacity}
              glassOpacityOptions={glassOpacityOptions}
              onChangeGlassOpacity={handleChangeGlassOpacity}
              manaPipStyle={manaPipStyle}
              manaPipStyleOptions={manaPipStyleOptions}
              onChangeManaPipStyle={handleChangeManaPipStyle}
              defaultCollectionSort={defaultCollectionSort}
              collectionSortOptions={collectionSortOptions}
              onChangeDefaultCollectionSort={handleChangeDefaultCollectionSort}
              compactCardsMode={compactCardsMode}
              onToggleCompactCardsMode={handleToggleCompactCardsMode}
              deckBoxFlair={deckBoxFlair}
              onToggleDeckBoxFlair={handleToggleDeckBoxFlair}
              bgMode={bgMode}
              bgPresets={bgPresets}
              onSetBgMode={handleSetBgMode}
              onOpenBgSearch={handleOpenBgSearch}
              onRemoveBgPreset={handleRemoveBgPreset}
            />
          )}
          ...
          {(!isSearching ? activeTab === 'connection' : (matchLogPath || matchBo3Sideboard || matchInstallLocations)) && (
            <ConnectionTab
              ...
              bo3SideboardTracking={bo3SideboardTracking}
              onToggleBo3Sideboard={handleToggleBo3Sideboard}
            />
          )}
          ...
          {(!isSearching ? activeTab === 'storage' : (matchDbStats || matchAutoBackup || matchImageCache || matchCacheQuota || matchCardDbSync || matchSetCatalog)) && (
            <StorageTab
              palette={palette}
              isSearching={isSearching}
              matchDbStats={matchDbStats}
              matchAutoBackup={matchAutoBackup}
              matchImageCache={matchImageCache}
              matchCacheQuota={matchCacheQuota}
              matchCardDbSync={matchCardDbSync}
              matchSetCatalog={matchSetCatalog}
              dbStats={dbStats}
              autoBackupEnabled={autoBackupEnabled}
              dbExporting={dbExporting}
              dbExportSuccess={dbExportSuccess}
              onToggleAutoBackup={handleToggleAutoBackup}
              onExportDb={handleExportDb}
              cacheStats={cacheStats}
              imageCacheQuota={imageCacheQuota}
              imageCacheQuotaOptions={imageCacheQuotaOptions}
              cacheDownloading={cacheDownloading}
              cacheClearing={cacheClearing}
              downloadProgress={downloadProgress}
              cacheClearSuccess={cacheClearSuccess}
              onChangeImageCacheQuota={handleChangeImageCacheQuota}
              onPreDownloadArt={handlePreDownloadArt}
              onClearCache={handleClearCache}
              avatarCacheStats={avatarCacheStats}
              avatarDownloading={avatarDownloading}
              avatarClearing={avatarClearing}
              avatarDownloadProgress={avatarDownloadProgress}
              avatarClearSuccess={avatarClearSuccess}
              onExtractAvatarsFromClient={handleExtractAvatarsFromClient}
          ...
          {(!isSearching ? activeTab === 'about' : (matchAboutSummary || matchLegal)) && (
            <AboutTab
              isSearching={isSearching}
              matchAboutSummary={matchAboutSummary}
              matchLegal={matchLegal}
              version={version}
            />
          )}
