...
export function HoverArtPreview({ card, palette }: HoverArtPreviewProps) {
  if (!card) {
    return (
      <div 
        className="h-64 rounded-none border border-dashed flex flex-col items-center justify-center p-4 text-center space-y-1 transition-all"
  ...
  return (
    <div 
      className="h-64 rounded-none border p-2 flex flex-col items-center justify-center overflow-hidden shadow-2xl relative transition-all group"
      style={{ backgroundColor: palette?.surface, borderColor: palette?.border }}
    >
      <img 
        src={imageUrl} 
        alt={card.name}
        className="h-full object-contain rounded-none drop-shadow-md transition-transform duration-200 group-hover:scale-105"
        onError={(e) => {
          // Fallback to placeholder text on image fail
          (e.target as HTMLElement).style.display = 'none';
        }}
      />
      <div className="absolute bottom-2 left-2 right-2 bg-black/80 backdrop-blur-md p-1.5 rounded-none border text-center truncate" style={{ borderColor: palette?.border }}>
